...
class PlatesController {
  async create(request, response) {
    ...
    if (user.admin !== 1) {
      throw new AppError(
        "Usuário não é administrador e não pode criar pratos."
      );
    }
  ...
  async update(request, response) {
    ...
    if (user.admin !== 1) {
      throw new AppError(
        "Usuário não é administrador e não pode editar pratos."
      );
    }
        ...
        .whereIn("dish.id", function () {
          this.select("tags.dish_id")
            ...
            .whereIn(
              ...
              filterIngredients.map((ingredient) => ingredient.toLowerCase())
            );
        })
        .orWhereLike("dish.name", `%${search}%`);
    } else {
      dish = await knex("dish").whereLike("name", `%${search}%`);
